...
STOPWORDS = {"the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for", "with", "is", "are", "was", "were", "it", "this", "that", "of", "from", "about", "by", "an", "as", "at", "if", "or", "so", "up", "out", "who", "get"}
...
def preprocess_text(text: str):
    ...
    # Stopword removal and lemmatization (simple version)
    cleaned = []
    for t in tokens:
...
def forecast_trend_rf(history_dates, history_volumes, forecast_days=30):
    """
    Uses a RandomForestRegressor to forecast keyword popularity.
    Fits a regressor on lag features (t-1, t-2, t-3) of historical volumes.
    """
    if len(history_volumes) < 7:
        # Fallback to random walk + trend if history is too short
        last_val = history_volumes[-1] if history_volumes else 10
        forecast = []
        for i in range(forecast_days):
            change = random.uniform(-2, 5)  # slight rising bias
    ...
    # Calculate simulated confidence interval using standard deviation
...
def cluster_topics(dataset, n_clusters=6):
    """
    Clusters dataset keywords into groups using TF-IDF and KMeans.
    """
    if not dataset:
        return []
    
    # Get distinct keywords and their sample texts
    keyword_map = {}
    for item in dataset:
        kw = item["keyword"]
        text = item["text"]
    ...
    # Join documents for each keyword
    ...
    try:
        ...
        for c_id, kws in clusters.items():
            ...
            # Calculate sum of mentions for this cluster
